...
const Tvoverview = ({Movie, handlebook})=>{
      const [videourl , setvideourl] = useState([]);
      const navigate = useNavigate();
      var [loadings , setloadings] =useState(true)
 useEffect(() =>{
  fetch(`https://api.themoviedb.org/3/tv/${Movie.id}/videos?api_key=5f212fdc2e5c2bc385fb3506fe3f0e28&append_to_response=videos`).then(
    Response => Response.json()
  ).then(
    data => setvideourl(data.results)
  )
  setTimeout(()=>{
    setloadings(false)
  },2000)
...
const key = videourl.find(vid => vid.type === 'Trailer')
    ...
    return(
        ...
        {loadings ? <Loader/>  :
         <div className="mainoverview">
             ...
             <div className="video">
               <YouTube videoId={key? key.key : null} opts={opts}/>
             </div>
